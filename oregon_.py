				...
				if msg["req"] == "res":
					...
					if transactions_total_amount > msg["ledger"]:
						...
						if supporter is not None:
							sendMessage(json.dumps({"req":"supporter", "supporterName": supporter, "amount":supportingAmount}), block.owner)
						else:
							validity=False
...
class Verifier():
	...
	def proofOfStake(self, miner, supportingAmount):
		global blockGenProbability
		# print(blockGenProbability)
		for supporter in self.ledger.keys():
			if supporter != miner and self.ledger[supporter] >= supportingAmount:
				# print(supporter in blockChain.chainOwnerStat.keys())
				# print((blockChain.chainOwnerStat[supporter]+1)/len(blockChain.chain) > blockGenProbability)
				# print((blockChain.chainOwnerStat[supporter]+1)/len(blockChain.chain))
				if supporter in blockChain.chainOwnerStat.keys() and (blockChain.chainOwnerStat[supporter]+1)/len(blockChain.chain) > blockGenProbability:
					sendMessage(json.dumps({"req":"supporter exceeds p", "supporterName": supporter}), miner)
				else:
					return supporter
		return None
...
#			print(doubleSpendingBlock)
			...
			block_miner_msg = {"req":"verify this block", "block.owner": doubleSpendingBlock.owner, "block.prev_hash": doubleSpendingBlock.prev_hash, "block.nonce": doubleSpendingBlock.nonce, "block.transactions": doubleSpendingBlock.transactions.asJson(), "block.final_hash": doubleSpendingBlock.final_hash, "ledger":ledger}
			sendMessage(json.dumps(block_miner_msg), verifier)
			print("New block has been sent to other verifiers")
		elif command == "debug":
			...
			# 	newBlock.generateHash()
		# 	newBlock.commit()
			print(blockChain.chainOwnerStat)
			print(verifier_c.ledger)
		else:
