			...
			if msg is None:
				...
				if msg["req"] == "res":
					...
					if transactions_total_amount > msg["ledger"]:
						...
						if verifier_c.proofOfStake(miner=block.owner, supportingAmount=supportingAmount) is not None:
					...
					if validity:
						...
						sendMessage(json.dumps({"req":"reward", "type":"verifying", "amount":transactions_total_amount*verifying_reward_percentage}), block.owner)
					else:
					...
					sendMessage(json.dumps({"req":"reward", "type":"signing", "amount":msg["amount"]*signing_reward_percentage}), block.owner)
